...
static uint32_t os_mtime_per_tick;
void OSInitHook(void)
{
  INT_SetMtimeHi(0);
  INT_SetMtimeLo(0);
  os_mtime_per_tick = SYS_GetSysClkFreq() / OSCfg_TickRate_Hz;
  INT_SetMtimeCmp(os_mtime_per_tick);
  INT_Init();
  INT_EnableIntGlobal();
...
void MTIMER_isr(void)
{
    ...
    INT_SetMtimeCmp(INT_GetMtimeCmp() + os_mtime_per_tick);
...
{
  return ((CPU_TS_TMR)read_csr(mcycle));
}
#endif
...
{
  return UTIL_McycleToUs(ts_cnts);
}
#endif
